...
class Controller extends \App\Library\Payments\BaseController
{
	...
	public function callbackRoute(Request $request)
	{
		...
		try
		{
			...
			if (!empty($json['method']) &&
				isset(self::$request_params[$json['method']])
			)
			{
	...
	public function checkAuth()
	{
		...
		if (!$user || $user != self::config('username') || $pass != self::config('password'))
		{
			// $title='Access denied.';
			// if($user)
			// 	$title.=" user";
	...
	{
		$transaction=Transaction::where('system_transaction_id', $this->parameters['id'])
		...
		if (!$transaction)
		{
			...
			$transaction=Transaction::create([
					'system_transaction_id'=>$this->parameters['id'],
					'paycom_system_time_created'=>$this->parameters['time'],
					'amount_tiin'=>$this->parameters['amount'],
					'amount'=>$this->getAmount(),
					'account_id'=>$this->getAccountId(),
					'parameters'=>json_encode($this->parameters['account']),
	...
	public function actionGetStatement()
	{
		$transactions=Transaction::select([
				'system_transaction_id as id',
				'id as transaction',
				'paycom_system_time_created as time',
				'amount_tiin as amount',
				'parameters',
	...
	public function actionChangePassword()
	{
		return
		[
			"success" => $this->changePassword()?true:false,
		];
	}
	...
	public function test()
	{
		...
		$array=
		[
			"merchant"=>"56a07e858cb0d102137f29cf",
			"amount"=>"10000",
			"account"=>[
				"request"=>"1",
			],
